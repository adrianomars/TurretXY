...
/*	I/O List
    ...
    PA4  : CS        : GPIO
    PB4  : DC        : Alternative function 5		
    PA0  : ADC1_CH5  : GPIO, Analog Input
    PA5  : TIM2_CH1  : Input Capture AF1	
	PA8  : TIM1_CH1  : PWM Output AF1    
    PA9  : TIM1_CH2  : PWM Output AF1
    PA2  : USART2_TX : Serial AF7
    PB0  : ADC1_CH15 : GPIO, Analog Input		
	PB3  : TIM2_CH2  : Input Capture    
    PB5  : EXTI5     : GPIO, Digital Input, Pull-up enabled
*/
...
// EXTI5 Interrupt
void initDisplayButton(void); // Initialize EXTI5 to PB5

// Display
void displayFunctions(void); // Display mode logic
void dispStats(void); // Used to display measurements in display
...
#define POT_ADC_MIN 0     // measured min ADC count for potentiometer
#define POT_ADC_MAX 4000  // measured max ADC count for potentiometer
...
int main()
{
    ...
    while (1)
    {
        // This function resets servos to 90 degrees upon startup. Intended for joystick
        if (!calibrated)
            {
...
// [v1.0.2] - used same variable to make it easier to adjust
uint16_t pwmToAngle(uint16_t pwm_ticks)
{
    ...
    return (pwm_ticks - SERVO_PWM_MIN) * 180 / (SERVO_PWM_MAX - SERVO_PWM_MIN);
}
...
void initADC(void)
{
    ...
    ADC1->SQR1 |= (5 << 6) | (15 << 12); // Set channel 5 as first conversion, and channel 15 as second
...
// [v1.0.2] - moved starting CCRx values into an if-statement with a flag so they would not impact the calibration
void servo_startup_ramp_stepper(void)
{
    ...
    if(!initTIM1valsFLAG)
    {
        ...
        TIM1->CCR1 = 60;
        TIM1->CCR2 = 60;
        // Set flag to mark default values assigned
        initTIM1valsFLAG = 1;
    }
...
// [v1.0.2] - renamed state variables to be more clear
void TIM2_IRQHandler(void)
{
    ...
    static uint8_t state_Y = 0, state_X = 0; // State keeps track if waiting for rising or falling edge
