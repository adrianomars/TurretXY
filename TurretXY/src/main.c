...
#define SERVO_PWM_MAX 119 // corresponds to ~2.4ms pulse

#define POT_ADC_MIN 5     // measured min ADC count for potentiometer
...
int main()
{
    ...
    while (1)
    {
        // This function resets servos to 90 degrees upon startup.
        if (!calibrated)
            {
...
{
    // 30 ticks = 0 degrees
    // 118 ticks = 180 degrees (decided this through tuning, for some reason its not the same as max ADC count)
...
void initADC(void)
{
    ...
    ADC1->CFGR = 0; // Reset ADC configure register
    ...
    ADC1->SMPR1 = (6 << (3 * 5)) | (6 << (3 * (15-10))); // Longer sampling time

    ADC1->CR |= (1 << 0);
    while (!(ADC1->ISR & (1 << 0))); // Wait for ADC ISR to be on
}
...
void initTIM1_PWM_Servo(void)
{
    ...
    selectAlternateFunction(GPIOA,9,1);
...
{
    TIM1->CCR1 = 60; // Initial Servo Y PWM ticks
    TIM1->CCR2 = 60; // Initial Servo X PWM ticks
...
// PB5 Display Cycle Button Initilizer
