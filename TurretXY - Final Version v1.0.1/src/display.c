...
/*	I/O List
        ...
        PB4  : D/C : GPIO Output (changed from PA5)
		PA4  : CS : GPIO Output
...
{
    pinMode(GPIOA,6,1);
    pinMode(GPIOB,4,1); // Changed DC output to PB4
    pinMode(GPIOA,4,1);
    initSPI(SPI1);
...
static void DCHigh(void) // Had to change DCHigh and DCLow to fit TIM2 Capture
{
    GPIOB->ODR |= (1 << 4);
}
static void DCLow(void)
...
void fillRectangle(uint16_t x,uint16_t y,uint16_t width, uint16_t height, uint16_t colour)
{
    ...
    waitForSPIReady(); // Check that it is ready
}
void putPixel(uint16_t x, uint16_t y, uint16_t colour)
{
...
void drawSemiCircle(uint16_t x0, uint16_t y0, uint16_t radius, uint16_t Colour)
{
    ...
    // Check that the entire semicircle will be on screen
    if (radius > x0)
        return;
    if (radius > y0)
        return;
    ...
    while (x >= y)
    {
        // Only draw points from top half
        putPixel(x0 + x, y0 - y, Colour);
        putPixel(x0 + y, y0 - x, Colour);
        putPixel(x0 - y, y0 - x, Colour);
        putPixel(x0 - x, y0 - y, Colour);
    ...
    // Draw the semi-circle base
    drawLine(x0 - radius, y0, x0 + radius, y0, Colour);
}
...
// This function only displays the digits used to reduce the number of pixels updated
void printNumberSHORT(uint16_t Number, uint16_t x, uint16_t y, uint16_t ForeColour, uint16_t BackColour)
{
    char Buffer[6];
    uint8_t i = 0;

    fillRectangle(x, y, 5 * FONT_WIDTH, FONT_HEIGHT, BackColour); // Remove extra digit when going from a higher number of digits to less

    if (Number >= 10000) Buffer[i++] = '0' + (Number / 10000) % 10; // If 5 digits, print each
    if (Number >= 1000 || i > 0) Buffer[i++] = '0' + (Number / 1000) % 10; // If 4 digits, print each
    if (Number >= 100 || i > 0) Buffer[i++] = '0' + (Number / 100) % 10; // If 3 digits print each
    if (Number >= 10 || i > 0) Buffer[i++] = '0' + (Number / 10) % 10; // If 2 digits, print each
    Buffer[i++] = '0' + (Number % 10); // Always print 1st digit
